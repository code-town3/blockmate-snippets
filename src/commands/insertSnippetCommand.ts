...
export class InsertSnippetCommand {
    ...
    async execute(): Promise<void> {
        ...
        if (snippets.length === 0) {
            vscode.window.showInformationMessage('No snippets available for this file type');
            return;
        }
    ...
    private async showSnippetPicker(snippets: any[]): Promise<any | undefined> {
        const items = snippets.map(snippet => ({
            ...
            detail: `Prefix: ${snippet.prefix} | Tags: ${snippet.tags.join(', ')} | Usage: ${snippet.usageCount}`,
            snippet: snippet
        }));
